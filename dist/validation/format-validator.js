...
const validatePlusTime = (expr, values) => {
    ...
    if (values[1] !== undefined && !(0, internals_1.isInt)(values[1])) {
        errors.push({ expr, message: `"amount" argument (#2) of "plusTime" must be an integer, but it is: ${values[1]}` });
    }
    if (values[2] !== undefined && typings_1.timeUnits.indexOf(values[2]) === -1) {
        errors.push({ expr, message: `"unit" argument (#3) of "plusTime" must be a string equal to one of ${typings_1.timeUnits.join(", ")}, but it is: ${values[2]}` });
    }
    return errors;
};
...
const validateExtractFromUVCI = (expr, values) => {
    const errors = [];
    if (values.length !== 2) {
